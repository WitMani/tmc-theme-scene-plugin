...
class ScaleCalibrateTests(unittest.TestCase):
    def setUp(self):
        ...
        body(90, 333).save(self.person_raw)  # far from 130 tall, odd ratio
        ...
        self.plan = {'H_px': 130, 'alpha_threshold': 16, 'items': [
    ...
    def test_calibrate_all_hits_plan_registers_and_binds_review(self):
        ...
        self.assertEqual(visible(self.run / person['artifact']['file'], 'height'), 130)
        self.assertEqual(visible(self.run / boat['artifact']['file'], 'width'), 585)
        ...
        self.assertEqual(entries['boat']['target_px'], 585)
        self.assertEqual(entries['boat']['actual_px'], 585)
    ...
    def test_recalibration_is_idempotent_and_voids_old_approval(self):
        ...
        self.assertEqual(again['actual_px'], 130)
        review = h.read_json(self.run / 'scale-review.json')
        self.assertEqual(review['status'], 'unreviewed')
    ...
    def test_plan_change_resets_review_binding(self):
        ...
        self.assertEqual(visible(self.run / boat['artifact']['file'], 'width'), 650)
    ...
    def test_cli_and_contact_sheet(self):
        ...
        with Image.open(out) as im:
            self.assertGreaterEqual(im.width, 585 + 16)
        self.assertEqual(h.main(['scale-contact', '--run', str(self.run), '--out', str(out)]), 2)
        with self.assertRaises(ValueError):
            contact_sheet(self.run, out)

    def test_legacy_run_keeps_128_scale(self):
        from scene_size import LEGACY_CONTRACT
        manifest = h.read_json(self.run / 'manifest.json')
        manifest['scene_size_contract'] = LEGACY_CONTRACT
        h.write_json(self.run / 'manifest.json', manifest)
        self.plan['H_px'] = 128
        h.write_json(self.run / 'scale-plan.json', self.plan)
        result = calibrate_asset(self.run, 'person')
        self.assertEqual(result['actual_px'], 128)
