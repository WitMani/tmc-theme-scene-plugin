...
- 默认采用**全量并发**：统一准备好输入后，一次性提交本次全部独立素材及背景调用。默认14个素材＋1张背景共15个请求；实际12–15个素材时共13–16个请求。不人为分成4张或8张批次，不等待前一张完成再启动后一张。用户指定更低并发或工具明确声明上限时遵从。此策略仅针对已选定任务，不增加素材数量或额外候选。
