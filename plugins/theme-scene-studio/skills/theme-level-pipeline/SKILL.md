...
各阶段按其技能完成一次集中检查后直接交接；编排层沿用图片/计划未变更且仍有效的结果，不再独立做一遍美术review或追加审核agent。跨阶段只检查必要接口与本阶段新增行为。游玩阶段仍检查实际功能，但不重审已通过且未变化的上游美术。仅明确失败时定向返工并复查受影响内容；轻微偏好差异记录备注，已有预算不是必须用满的审查轮数。通过即交付，不自动扩大问题范围或再跑全流程。
...
| 素材标尺 | 人物基体75×130 px，附件可超出；小物件150–300、中物件301–500、大物件501–830 px（宽高）；道路1–2.5车宽，大型实例上限x待场景配置 |
...
- 按当前技能做透明化、4096背景标准化、H=130尺度校准、美术与基础设施复核。所有复核绑定实际文件和计划哈希。
