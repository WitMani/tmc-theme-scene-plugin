...
1. 查看本次干净主图，选择统一的斜俯视正交目标方向。以主图整体镜头为依据；局部建筑方向冲突时不逐栋描线。没有可靠角度时使用清晰的方向描述，不冒充测量值、不固定所有主题为±30°或45°。用户指定镜头优先；非正交要求不强套平行网格。
...
4. 保存 `references/camera-guides.png` 与 `references/camera-guides.prompt.txt`（实际英文 prompt）。不同参考组分目录保存；修正加版本号保留历史。没有新主图或镜头要求时，不为每候选重复生成辅助线。
5. 候选输入顺序为原始1–3张干净图，最后附加辅助线图，共2–4张；原图不删减。若用户已经提供明确配对且可用的辅助线版，查看并复用，不重复派生。完整输入无法通过当前工具传入时说明缺失项并请求补齐，不拼图、不漏图。若用户明确禁用辅助线，记录 disabled 并沿用原有流程。
...
Target shared projection: [chosen common orthographic orientation; describe or give supported target directions, not invented measurements].
...
填写目标方向后才调用；候选生成使用 prompt-template.md 中的参考分工段，明确辅助线优先约束几何、干净图负责画风，且最终画面完全移除标记。辅助线不要求复制原图地形或限制三候选的布局变化。
