...
保持相同参考组；默认第一张主导，共性优先，不平均混合不兼容画风。干净原图只有一张时删除其他干净参考图的描述，但保留额外辅助线图及其用途。N表示干净原图数量，默认实际输入为N+2（干净图、辅助线图、当前草图）；填入真实编号。用户明确关闭辅助线时删除派生图与CAMERA GUIDE ROLE相关句子，并以主图选择镜头，保留当前布局草图并重新填写其编号（N+1）。不要把占位符直接发给生成器。斑驳是后续事项，不视为已解决。
