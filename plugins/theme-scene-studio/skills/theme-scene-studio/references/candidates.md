...
T个选定主题对应3T个槽位，干净原图N张（1≤N≤3，未输入时使用内置默认图一张），默认额外加入1张辅助线版，实际输入N+1张；辅助线准备不计入候选数量，参考图数量不乘入数量。状态分别记录pending/generated/reviewed/failed等，明确状态含义。只有图片文件真实存在且查看过才可标记已检查。若A/B/C出现重复、严重画风漂移或工具失败，对该槽位最多额外尝试一次，保留历史。只有完成了全部槽位才报告全部完成。
