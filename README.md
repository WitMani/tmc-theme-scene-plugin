...
| Stage 2 · 背景与素材 | [theme-stage2](plugins/theme-stage2/) | 一张选定的场景图 | 1 张背景 + 22 种独立素材 PNG（建筑8、道具6、载具3、角色5） |
...
每个主题先设计并审核三张布局草图，再输出三张场景候选；点击区占主体，非点击环境占少量。草图默认采用45°方位角、45°俯角的统一正交投影，地形、道路、圆井与建筑占位由同一XYZ投影脚本生成并校验。仅使用中性低细节占位，避免草图改变原图画风。每张成品使用对应草图作为空间参考。未附图时使用 Stage 1 自带参考图。
...
```text
使用 $theme-stage2，处理这张场景图，输出独立背景和默认22种素材 PNG。
```

### 3. 获取独立 PNG

Stage 1 默认规划并呈现22种独立原型；Stage 2 按建筑8、道具6、载具3、角色5逐类提取验收。用户明确种类要求优先，源图缺类须报告，不能跨类凑数或无依据补造。这项规则仅约束种类，不规定重复摆放或目标款数量。每个对象独立生成，背景另外生成；内部保留调用与复核记录，默认交付只有 PNG。缺少源图时会要求补充。

Stage 2 正式版采用美术规则 **1.6.0**：背景采用 B 档大形体与简洁明暗，内部优先留出连续可排布空间；非交互装饰减量并向画布外缘归并，道路、岔口和桥头保持净空，保留水流、岸线和桥体；素材沿用 **V2** 的邻近色描边、克制但可读的体积和卡通夸张。V3 单次试验的反面参考与加强 Prompt 不作为默认输入。
